...
// The whole Projects view shares one DndContext; cada draggable declares what it
// es por lo que collision detection y drag-end dispatch puede rama per drag tipo.
...
export const projectsViewCollisionDetection: CollisionDetection = (args) => {
    ...
    if (activeData?.type === 'project') {
        // Project drags stay inside su own sidebar section, matching la old
    ...
    if (activeData?.type === 'task') {
        ...
        if (pointerCollisions.length > 0) {
            // Prefer sidebar project rows sobre la surrounding area block.
