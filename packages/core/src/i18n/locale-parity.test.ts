...
describe('locale parity', () => {
    ...
    // The ADHD screens render through tFallback, which masks a missing key with
    // English — the exact regression story 1.6 shipped with in nine locales, and
    ...
    // tdahToday.*/tdahActivity.*/tdahPeriod.*/tdahHistory.*/tdahMetrics.* set.
    // Extend this prefix list whenever a new ADHD-screen namespace ships.
    const tdahScreenPrefixes = ['tdahToday.', 'tdahActivity.', 'tdahPeriod.', 'tdahHistory.', 'tdahMetrics.'];
    it.each(locales)('keeps every ADHD today/activity/history/metrics key translated in %s', (lang) => {
