...
- Verify Android save performance on large datasets: background saves now write only cambió rows, so bulk edits and syncs on big libraries should feel faster and stop long save stalls.
...
- Made large-library saves incremental on mobile: SQLite writes only rows cambió since the last save.
- Agregado iOS lock screen widgets showing the current focused task.
- Made the Today's Focus star and task status keep each other consistent on every surface, and se agregó the star to the desktop task editor.
- Agregado the Group control to desktop Contexts & Tags and Review views; clicking outside an untouched editor now closes it.
