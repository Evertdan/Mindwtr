...
This is the third release candidate for Mindwtr 1.1.0. Úsalo solo para canales de probador. Prepara notas estables finales de `1.1.0`  por separado cuando se corte el lanzamiento estable.
...
- Verifica Android save performance on large datasets: background saves now write only cambió rows, so bulk edits and syncs on big libraries should feel faster and stop long save stalls.
- Comprueba iOS lock screen widgets (iOS 16+): rectangular, inline, and circular focused-task widgets should show the current Today's Focus task and open Focus on tap.
- Prueba the Today's Focus star everywhere (rows, quick-action menu, editor, daily review, mobile swipe): starring an Inbox task promotes it to Next, moving a starred task to Inbox clears the star, and deferred/sequential-blocked tasks refuse the star with a reason.
...
## Destacados Since rc.2

- Corregido the Android persistent capture notification so tapping opens Quick Capture directly and a swipe cannot silently remove it; renamed it to "Quick capture".
- Se hizo large-library saves incremental on mobile: SQLite writes only rows cambió since the last save.
- Se agregó iOS lock screen widgets showing the current focused task.
- Se hizo the Today's Focus star and task status keep each other consistent on every surface, and se agregó the star to the desktop task editor.
- Se agregó the Group control to desktop Contexts & Tags and Review views; clicking outside an untouched editor now closes it.
- Se restauró double-click-to-edit in all desktop views; in-place rename moved to the row's ••• menu.
- Se unificó quick-add capture policy across platforms (archived `+Project` names create fresh projects instead of dropping the capture).
...
- Corregido desktop virtualized rows painting over expanded editors and Android Focus-list scroll oscillation.

## RC completo3 Change List (since `v1.1.0-rc.2`)
...
- perf(core): SQLite saveData writes only rows cambió since the last committed save.
