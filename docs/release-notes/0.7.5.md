...
## Destacados
- Agregado safer desktop list deletion flows with confirmation, undo, and clearer restore failure feedback.
- Agregado related-task navigation on mobile and trimmed task list selector churn for smoother everyday use.
...
- Expanded release and packaging readiness with Linux desktop metadata, Flathub automation, and F-Droid-safe Android build manejo.

## Escritorio
- Agregado confirmation and undo for list deletions and surfaced batch undo restore failures.
- Kept Integrations settings stable during edits and blocked checklist edits on reference-only items.
- Agregado Linux packaging metadata, AppStream brand colors, richer AppStream copy, and automated Flathub follow-up updates.
- Tightened desktop filtering feedback and localized desktop delete toasts.

## Móvil
- Agregado related-task navigation so linked tasks open the correct mobile view.
- Switched task meta navigation to public APIs and se corrigió same-view navigation regressions.
...
- Hardened sync behavior around restore flows, cloud mutation manejo, and cross-device consistency.

## Localization, Docs & Lanzamiento Tooling
- Localized invalid date command errors and refreshed changelog and roadmap docs.
- Resolved CI typecheck regressions and se actualizó Android release setup for current tooling.
