...
- Mejorado desktop behavior on Linux and AppImage setups with live system-theme updates, safer blur/focus sync behavior, and better manejo of blocked filesystem stat calls.
...
- Tightened merge and validation rules across core sync, SQLite persistence, cloud auth rate limiting, and MCP quick-add input manejo.
...
## Móvil
- Restored the Project field in the mobile task editor when project soporte is enabled.
- Hardened Android speech-to-text, Whisper loading, and OpenAI transcription diagnostics, and se corrigió speech updates to use the created task ID.
- Cleared stale sync conflict details after later sync failures and se corrigió Blob typing for speech upload helpers.
...
- Agregado regression coverage around SQLite project delete behavior.
- Capped MCP `quickAdd` input length and aligned validation with trimmed input manejo.

## Docs, Packaging & Lanzamiento
- Hardened Docker defaults with explicit auth-token configuration, pinned Bun images, and compose resource limits.
- Clarified wiki/docs coverage for Docker cloud REST API soporte and the current sync merge behavior.
