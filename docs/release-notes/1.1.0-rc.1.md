...
Este es el primer candidato de lanzamiento para Mindwtr 1.1.0. Úsalo solo para canales de probador. Prepara notas estables finales de `1.1.0` por separado cuando se corte el lanzamiento estable.
...
- Prueba the GitHub prerelease desktop artifacts on Linux, macOS, and Windows: launch, capture, edit, complete, delete, and reopen previous-stable data.
- Prueba project Trash purge and archived-project restore sync on Apple devices after the production CloudKit schema fields are confirmed.
- Comprueba mobile capture, Inbox, task editing, Markdown typing helpers, review flows, and large-text list readability.
- Comprueba AI provider setup, structured output manejo, speech-to-text fallback behavior, and MCP/cloud task write paths.
...
- Usa refreshed AI providers with stricter structured outputs, bounded retries, and se actualizó default models.
...
- Prueba a stronger RC path with Play internal and open testing uploads, parallel Android FOSS builds, Flathub beta, AUR beta, and GitHub prerelease artifacts.

## RC completo Change List (since `v1.0.5`)
...
- fix(trash): show deleted projects in Trash and soporte deleting them forever.
...
- feat(tasks): improve add controls, entity creation affordances, list grouping, and quick-add focus manejo.
...
- feat(ai): modernize OpenAI, Anthropic, and Gemini provider manejo with stricter structured output parsing and bounded retries.
