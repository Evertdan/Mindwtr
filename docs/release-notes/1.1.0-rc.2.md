...
This is the second release candidate for Mindwtr 1.1.0. Úsalo solo para canales de probador. Prepara notas estables finales de `1.1.0`  por separado cuando se corte el lanzamiento estable.

## RC Pruebas Focus

- Verifica manual sync on desktop and mobile always reads the remote document, even when the unchanged-check cache says nothing cambió.
- Confirm se corrigió-area and follow-active-area default task area settings sync between devices.
...
- Prueba desktop undo after completing a Today task, short-list pull-to-refresh on mobile, and MCP install on Windows with Node 22+.
...
## Destacados Since rc.1

- Corregido forced sync on desktop and mobile so "Sync now" and pull-to-refresh do not trust a stale fast-check cache.
- Corregido synced default area settings so new-task area preferences upload with the sync document.
- Se agregó an optional Android persistent "Quick add" notification for one-swipe capture from anywhere.
- Mejorado mobile task-row press feedback, Samsung emoji-label rendering, and selectable task descriptions.
- Mejorado desktop task editing with inline title rename, selectable expanded descriptions, reliable area/project drag targets, and project-sidebar task moves.
- Corregido recurrence and scheduling edge cases, including date-less recurrence regeneration and relative start offset `0`.
- Actualizado `mindwtr-mcp` (1.1.1) so global npm installs can use current prebuilt `better-sqlite3` binaries on Node 22+ (Node 20 still works but needs build tools).
- Se mantuvo MCP package versions independent from app release version stamping so app RCs cannot overwrite npm package fixes.

## RC completo2 Change List (since `v1.1.0-rc.1`)
