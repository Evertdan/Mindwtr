...
- Mejorado mobile sync UX with a global sync status indicator, sync connection checks, dedicated empty states, and better accessibility labels.
...
- Hardened security and configuration paths with explicit cloud any-token opt-in and stricter MCP write-mode CLI manejo.
...
- Agregado retry tracking for failed remote attachment cleanup attempts.
- Corregido store flush behavior to continue draining newer queued saves after failures.
- Preserved task status when restoring deleted tasks.
- Mejorado merge edge-case manejo for invalid `deletedAt`, explicit empty attachment arrays, and deep-cloned settings merges.
...
## Móvil / Escritorio UX
- Agregado mobile fail-safe boundary manejo for task edit modal audio lifecycle errors.
- Agregado mobile sync status indicator, sync connection checks, and empty-state UI coverage.
- Mejorado mobile accessibility labels and se eliminó hardcoded status fallback labels.
- Corregido stale sync error banners showing after switching sync backend to Off.
- Continued desktop quality improvements (archived status soporte, tray/dock behavior fixes, UI polish).

## Tooling / Docs / Tests
- Enabled sqlite-adapter test execution in core vitest runs.
- Aligned ESLint and TypeScript workspace versions.
- Corregido desktop CI hangs by ensuring CI test scripts run in non-watch mode.
- Actualizado docs and wiki references (roadmap links, developer guide table fixes, contribution docs cleanup).

## Historial completo de cambios (since 0.6.16)
