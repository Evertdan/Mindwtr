...
- Móvil polish is broader across Material 3 tokens, filled CTA buttons, FABs, card ripples, keyboard-covered popups, large text wrapping, toasts, review flows, and widget readability.
...
- Agregado bulk Quick Add capture and localized its strings.
- Kept typed quick-add text by default, with an opt-in auto-clean setting for stripping detected tokens.
- Agregado a TickTick backup importer and preserved archived status during import.
- Agregado migration docs plus a CSV-to-QuickAdd helper script.
- Agregado a toggle to disable Markdown typing helpers and limited automatic link/code character closing to active typing.
- Ignored duplicate mobile Markdown keypress echoes.
- Agregado desktop reference organization fields and focused popup task-editor titles.

## Planning, Review & GTD

- Agregado repeat reminders for due-time tasks, including core persistence, sync signatures, cloud writes, i18n, desktop polling, and mobile scheduled alarms.
- Agregado board filters for context, tags, and dates, backed by shared filter criteria.
- Agregado tag grouping in Focus and se actualizó the mobile Focus grouping view.
- Agregado a default task area setting and preserved task area when clearing project assignments.
...
- Agregado Apple Shortcuts v1 actions and an Apple Shortcuts guide.
...
- Pinned project bulk-selection actions and se mejoró bulk-organize actions.
- Agregado swipe-dismiss to toasts and restored quick status changes as an icon button on single-status lists.
- Mejorado large-text wrapping with a shared scale cap.
- Agregado Material 3 color, type, shape, elevation, state-layer, text-role, filled-button, FAB, card ripple, and CTA button soporte behind self-gating tokens.
- Excluded git worktrees from Metro watchers.

## Sync, Cloud, MCP & Automation

- Agregado a read-only Cloud backend for the MCP server and aligned Cloud search semantics.
...
- Agregado a shared calendar-push event content builder and enriched mobile/desktop calendar push events while serializing writes to prevent duplicates.
- Reflected Mindwtr calendar color changes in external Android calendar apps.
- Agregado performance diagnostic log generation in core.

## Lanzamiento, Docs & Calidad

- Agregado the release candidate workflow and Linux beta RC channels for GitHub prerelease, Flathub beta, and AUR beta testing.
...
- Agregado RC-aware analytics heartbeat versioning so pre-release builds report `1.0.5-rc.1` separately from stable `1.0.5`.
...
- Agregado ADR 0018 for the mobile theming token hook.
