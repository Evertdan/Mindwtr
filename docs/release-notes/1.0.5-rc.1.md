...
Este es el primer candidato de lanzamiento para Mindwtr 1.0.5. Es para canales de probador; las notas finales estables de `1.0.5` deben prepararse por separado cuando se corte el lanzamiento estable.

## Enfoque de Pruebas RC

- La distribución del probador de Android debe cubrir tanto pruebas internas como pruebas abiertas cuando está habilitada la carga de Play.
- Verifica the RC blocker fixes: Inbox Processing `Later` + `No date`, Focus default grouping, capture default-area picker, docs-link routing, desktop task overflow-menu selections, and AI weekly review project moves to Someday.
- Comprueba release automation recovery paths: Docker duplicate-tag skips, stable-only MCP publishing, manual RC redispatch after retagging, Play versionCode auto-bump recovery, and stable handoff to tester channels.
...
- Capture and import are more capable with bulk Quick Add capture, safer quick-add token manejo, a TickTick backup importer, and a CSV helper for migration workflows.
...
- El refinamiento móvil is broader across Material 3 tokens, filled CTA buttons, FABs, card ripples, keyboard-covered popups, large text wrapping, toasts, review flows, and widget readability.
- Sincroniza and storage are more defensive with parent-reference repair, invalid cloud reference rejection, clearer native/WebDAV failure causes, PWA auto sync, and steadier startup/project state.
- Lanzamiento quality se mejoró with the RC workflow, Linux beta tester channels, safer version bump manejo for RC tags, RC-aware heartbeat versions, Play tester-channel handoff, performance diagnostic logs, public-doc routing cleanup, and broader regression coverage.

## Capture, Import & Editing

- Se agregó bulk Quick Add capture and localized its strings.
- Se mantuvo typed quick-add text by default, with an opt-in auto-clean setting for stripping detected tokens.
- Se agregó a TickTick backup importer and preserved archived status during import.
- Se agregó migration docs plus a CSV-to-QuickAdd helper script.
- Se agregó a toggle to disable Markdown typing helpers and limited automatic link/code character closing to active typing.
- Ignored duplicate mobile Markdown keypress echoes.
- Se agregó desktop reference organization fields and focused popup task-editor titles.

## Planning, Review & GTD

- Se agregó repeat reminders for due-time tasks, including core persistence, sync signatures, cloud writes, i18n, desktop polling, and mobile scheduled alarms.
- Se agregó board filters for context, tags, and dates, backed by shared filter criteria.
- Se agregó tag grouping in Focus and se actualizó the mobile Focus grouping view.
- Se agregó a default task area setting and preserved task area when clearing project assignments.
- Routed the desktop add-task shortcut to the current view and se agregó a desktop slash-command title picker.
- Corregido recurrence previews to show projected dates.
- Se mantuvo daily and weekly review progress when tapping task chips.
- Ordenaed completed project tasks by completion time and preserved project scroll state on desktop.

## Móvil & Apple Plataforma Polish

- Se agregó Apple Shortcuts v1 actions and an Apple Shortcuts guide.
- Corregido the Apple Shortcuts Open List default so TestFlight and App Store archives compile under Xcode 26.
- Mejorado iOS widget readability and se agregó iPad extra-large widget soporte.
- Se estabilizó mobile startup project/task state, Focus hydration, Focus row refresh after completion, and task-list layout after refresh.
- Lifted Inbox processing and other keyboard-covered popups across screens.
- Se fijó project bulk-selection actions and se mejoró bulk-organize actions.
- Se agregó swipe-dismiss to toasts and restored quick status changes as an icon button on single-status lists.
- Mejorado large-text wrapping with a shared scale cap.
- Se agregó Material 3 color, type, shape, elevation, state-layer, text-role, filled-button, FAB, card ripple, and CTA button soporte behind self-gating tokens.
- Excluded git worktrees from Metro watchers.

## Sync, Cloud, MCP & Automation

- Se agregó a read-only Cloud backend for the MCP server and aligned Cloud search semantics.
- Se expuso native request failure details and preserved WebDAV fetch error causes.
- Se habilitó PWA auto sync.
- Repaired missing parent references before persistence and rejected invalid task references before cloud repair.
- Se agregó a shared calendar-push event content builder and enriched mobile/desktop calendar push events while serializing writes to prevent duplicates.
- Reflected Mindwtr calendar color changes in external Android calendar apps.
- Se agregó performance diagnostic log generation in core.

## Lanzamiento, Docs & Calidad

- Se agregó the release candidate workflow and Linux beta RC channels for GitHub prerelease, Flathub beta, and AUR beta testing.
- Se hizo the version bump script safe for RC tag input while keeping app/package versions on the stable base version.
- Se limpió npm publish configuration and reduced redundant desktop CI setup work.
- Se agregó RC-aware analytics heartbeat versioning so pre-release builds report `1.0.5-rc.1` separately from stable `1.0.5`.
- Permitir Android release CI auto-raise the Play `versionCode` when the store already has a higher code, and reuse stable production builds on internal and open testing tracks.
- Actualizado README install routing, public docs links, IzzyOnDroid/Snap install badges, and About links.
- Se agregó ADR 0018 for the mobile theming token hook.
...
## RC completo Change List (since `v1.0.0`)
...
- refactorización(filters): use shared FilterCriteria
...
- refactorización(mobile): delegate useThemeColors to useThemeTokens
