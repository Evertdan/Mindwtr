...
- Móvil capture overlay now fills the screen correctly even with the keyboard open.
- Escritorio checklist items in the details view can now be checked off directly.
- Attachment sync is more reliable by ensuring attachment files and `cloudKey`s are prepared before sync writes.
- Special thanks to **@karl1990** for sponsoring the project. Your soporte helps keep Mindwtr free and open.
...
- Enable the Tauri FS watch característica needed by the sync file watcher.
...
- Improve notification permission manejo when the system requires opening settings.
- Improve sync reliability by pre-syncing attachments before the main sync write.
...
## Historial completo de cambios (since 0.6.1)
