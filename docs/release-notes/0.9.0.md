...
- Móvil settings and status surfaces are easier to scan with GTD sub-screens, clearer sync-error status, and delete undo for task list/edit flows.

## Focus, Calendar & Reminders

- Hid future-start tasks by default and se agregó a compact "hidden future start" affordance where the filter applies.
- Split start and due reminders into separate notification streams and se corrigió review notification alarm keys.
- Agregado mobile week calendar density, restored month swipe behavior, opened week calendar on the selected day, and documented what the density scale means.
- Let mobile users complete tasks directly from notifications.
- Made Pomodoro task linking optional, separated timer and task actions, se agregó link-task localization fallbacks, and preserved selected task context across link-toggle round trips.
...
- Agregado a desktop inbox edit-delete action.
- Agregado a task age visibility setting.
...
- Agregado ADRs for area soft-delete cascades, split start/due reminders, and the planned shared sync orchestration boundary.
- Documented the tombstone purge consistency bound and delete-vs-live diagnostics in the sync algorithm wiki.
- Agregado desktop Pomodoro axe coverage and native mobile accessibility metadata for week density controls.
