...
- Better desktop/mobile UX and accessibility (delete confirmation + undo, se mejoró labels, localized placeholders, loading skeletons, and larger mobile touch targets).
- Large refactorización and testability push (sync orchestration coverage, cloud API integration tests, shared sync runtime helpers, and component splitting).
- Linux desktop polish: native top menu is now macOS-only, and Linux app metadata/icon manejo was se mejoró.
...
- Agregado timeout/retry for cloud attachment uploads.
- Agregado task delete confirmation and undo toast recovery.
- Mejorado metadata/attachment accessibility labels.
- Agregado global search loading skeleton.
- Kept project/section/area selectors within viewport.
- Refactored task/project editor internals and split oversized modules.

## Móvil
- Agregado local-change sync abort protection and in-flight sync abort manejo.
- Agregado offline/network-drop abort behavior for sync HTTP requests.
...
- Agregado attachment endpoint-specific rate limiting.
- Agregado enforceable cloud bearer-token allowlist option.
...
- Agregado desktop and mobile sync orchestration tests.
- Agregado cloud API integration tests.
- Agregado desktop sync service reset hooks for test isolation.
...
- Unified attachment map/loop manejo across desktop sync backends.

## Historial completo de cambios (since 0.6.12)
