...
- Agregado custom time estimates and time estimate filters.
- Agregado project sequencing help cues and surfaced project deadline next actions in Focus.
...
- Agregado incremental task persistence for the desktop SQLite hot path.
- Agregado derived task indexes and virtualized large project task lists.
...
- Agregado a desktop task-menu action for moving tasks into Focus.
- Cleared Waiting prompt select focus more reliably.

## Sync & Attachments

- Agregado native iCloud attachment asset sync.
...
- Agregado portable Markdown toolbar shortcuts and soporte for Markdown links in the middle of email addresses.
- Avoided duplicate recurring follow-ups.
- Agregado external calendar colors.
- Agregado the Android context automation broadcast receiver.
...
- Agregado Android debug APK and profileable internal release workflows.
- Agregado the release profiling runbook and large-store performance budgets.
- Restored partial locale coverage and Markdown toolbar locale coverage.
- Agregado French store listings and refreshed French sync wording while keeping brand names untranslated.
