...
- Móvil preserves recovery snapshots when two files would receive the same label, and first-run starter data now waits for its save to finish.
...
- Escritorio sync isolates watcher lifecycles, ignores stale callbacks, binds credentials to the selected endpoint, and verifies the SQLite database before reusing a warm connection.
...
- Establecetings separates Backup from Import Data, while search opens the matching fold.
- Linux keeps the tray icon in the app cache so temp cleanup cannot leave an empty tray slot.

## Pruebas focus

- Actualizar from v1.1.6 or rc.1, create and edit tasks, restart the app, and confirm the changes persist.
- Trigger a recoverable storage failure, retry the save, and confirm the app keeps the newest snapshot and recovery history.
- Ejecuta two clients against self-hosted cloud sync, including concurrent edits and attachment deletion, then confirm a second sync produces no changes.
- Prueba Windows File Sync on a local folder and a WinFSP or rclone mount; confirm sync settles without lock errors or UI stalls.
...
- Prueba a sequential project whose first step is Waiting For and another whose later step has a start date.
