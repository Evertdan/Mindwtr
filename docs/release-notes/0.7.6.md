...
## Destacados
- Agregado more flexible processing and editing controls with guided/quick inbox modes, configurable task editor layouts, and a new Settings > Manage page for areas, contexts, and tags.
...
- Hardened sync reliability across core, desktop, mobile, cloud, and MCP with safer refresh manejo, better conflict resolution, stricter validation, and clearer error feedback.
- Mejorado desktop platform integration with macOS CloudKit sync, persisted iCloud Drive access, safer Obsidian vault detection, and better release/package metadata.

## Escritorio
- Agregado guided and quick inbox processing, flexible task editor layout controls, and se mejoró archived-task and bulk failure feedback.
- Agregado Settings > Manage for areas, contexts, and tags, plus bulk context editing soporte.
- Agregado first-pass macOS CloudKit sync and persisted iCloud Drive sandbox access across launches.
...
- Mejorado inbox/list interactions and surfaced auto-sync failures more clearly.
- Kept F-Droid Android builds compatible with current JVM and Gradle requirements and se mejoró reminder fallbacks.

## Sync, Cloud, MCP & Core
- Prevented stale refreshes from overwriting local edits and se mejoró delete-vs-live conflict resolution using revisions when available.
- Hardened attachment URI decoding, oversized Dropbox secret manejo, area filter persistence, and batch task/project validation.
- Agregado project and area management tools to the MCP server and rejected invalid MCP task statuses.
- Validated cloud task IDs in server routes and tightened cross-surface error reporting.

## Docs, Lanzamiento & Packaging
- Actualizado wiki coverage for CloudKit, Manage settings, inbox/task-editor settings, and release references.
- Mejorado Flathub assets, Flatpak and AUR release tooling, and F-Droid preparation scripts.
