...
- Móvil privacy improves with optional device app lock, while backup restore keeps that app-lock setting device-local.
...
- Agregado optional mobile app lock backed by device authentication.
...
- Agregado due-date sorting for project tasks.
- Grouped completed tasks below active work.
- Agregado and right-aligned mobile project task counts while decluttering project rows.
- Agregado desktop project task select mode.
- Agregado range selection soporte.
...
- Agregado quick add soporte in the calendar composer.
