...
- Móvil task lists gained filter controls, clearer filter chips, better Markdown snippets, and keyboard-safe Markdown/checklist editing.
- Escritorio project and task editing feels steadier with collapsible project navigation, stronger selected metadata pills, native spellcheck, and more reliable Markdown/checklist behavior.
- Plataforma integration is more reliable across Android context automation, repeated Siri capture, Flatpak tray behavior, GNOME theme detection, Windows dark chrome, and desktop tray activation.
...
- Agregado one-way macOS Mindwtr -> Apple Calendar push through EventKit.
- Agregado **Show next occurrence in Calendar** for recurring tasks as a planning-only preview.
- Preserved `showFutureRecurrence` across CloudKit, self-hosted cloud, SQLite, and sync helper paths.
- Agregado local ICS calendar sources on mobile.
...
- Agregado mobile task-list filters with polished controls, clearer chip labels, and better accessibility.
- Agregado Android context automation intents for activating and deactivating contexts from shortcuts and integrations.
...
- Made Markdown toolbar buttons adaptive and se corrigió icon sizing, including the italic control.
...
- Agregado a collapsible project panel.
