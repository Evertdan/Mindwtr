...
This is the fourth release candidate for Mindwtr 1.1.0. Úsalo solo para canales de probador. Prepara notas estables finales de `1.1.0`  por separado cuando se corte el lanzamiento estable.

## RC Pruebas Focus

- Intenta the new time tracking (#833): turn on the Pomodoro timer and "Link timer to task", complete focus sessions, and confirm each one credits the task's Time Spent value (editable next to the Time Estimate, synced across devices). With either toggle off, the field, row badge, and hover play button must stay completely hidden.
...
- Elimina tasks: they should move to Trash immediately with an Undo toast instead of a confirmation prompt, on both platforms; emptying Trash still confirms (#834).
- Comprueba the calendar week start follows your device locale out of the box, and the new "System default" option in Settings (#834).
- Verifica update reminders match your install channel (#829): winget/Chocolatey/Homebrew/AUR installs should only prompt when their channel has the release, Scoop installs should make no automatic checks, and Flatpak/Snap/App Store builds stay quiet.
...
- Escritorio update checks are channel-aware for every install channel, including quiet Scoop installs.
...
- Móvil project reorder drag moves tasks across sections in one flat list; task lists no longer settle-jump after a fling on Android.
- Estado badges, dark accents, and desktop scrollbars follow the active app theme.
...
## RC completo4 Change List (since `v1.1.0-rc.3`)
...
- fix(desktop): attachments se agregó in the task editor count as unsaved changes and can no longer be silently discarded or wiped by a stale draft. (#836)
...
- perf(mobile): widgets re-render only when their visible data cambió instead of on every save and sync. (#766)
