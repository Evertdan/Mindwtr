...
## Destacados
- Agregado mobile notifications for scheduled task times and project/task review times, following the same reliable pipeline as daily/weekly reminders.
...
- Agregado optional Pomodoro Focus timer on desktop and mobile, disabled by default to keep Focus view distraction-free.
...
- Corregido iOS quick-add keyboard/input spacing gap.
- Corregido audio attachment replay so tapping play after completion starts playback again.
- Mejorado iOS back/dismiss behavior by relying on native swipe-down modal dismissal flow.
- Actualizado iOS splash setup so configured splash sizing is applied correctly.

## Escritorio
- Mejorado Linux update download robustness across distribution paths.

## Lanzamiento & Distribution
- Agregado and hardened Mac App Store packaging/signing workflow.
- Agregado iOS App Store release workflow with Xcode 26 SDK soporte.
- iOS CI now resolves build numbers from local + App Store Connect max values to avoid duplicate upload failures.

## Historial completo de cambios (since 0.6.8)
