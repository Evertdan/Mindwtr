...
- Escritorio planning now supports dragging tasks onto the calendar, local ICS calendar files, a local API server toggle, task description previews, and stronger keyboard task selectors.
- Móvil capture and review workflows are more flexible with Siri inbox capture, a configurable quick access tab, optional Focus grouping, project-to-inbox conversion, and an Apple Reminders cleanup option.
...
- Sync, locale, and settings reliability se mejoró with Dutch locale exposure, locale coverage floors, heartbeat opt-out, safer attachment delete retries, unchanged-sync quieting, and file lock contention manejo.
- Escritorio polish includes system-theme defaulting, persistent WebView zoom, clearer global search selection reveal, se mejoró project split layout, metadata wrapping, and protected local API access.

## Escritorio Experience

- Agregado drag-to-calendar planning for desktop tasks.
- Agregado local ICS calendar file soporte and allowed Flatpak document-portal vaults.
- Agregado a local API server setting and protected local API calls with bearer tokens.
- Agregado desktop task description previews and a Done action in the task editor.
...
- Agregado Siri inbox capture.
- Agregado a configurable quick access tab.
- Agregado optional Focus grouping and clearer mobile task sort controls.
- Agregado inbox project conversion and an Apple Reminders cleanup option after import.
...
- Agregado project section reordering and preserved reference checklists.
...
- Opened day view from calendar overflow controls and se mejoró calendar drag accessibility.

## Sync, Settings & Data Safety

- Agregado an analytics heartbeat opt-out setting.
...
- Aligned current característica references in docs and expanded automation helper packaging notes.
