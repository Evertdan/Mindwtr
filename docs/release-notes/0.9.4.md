...
- La planificación de escritorio ahora admite arrastrar tareas al calendario, archivos de calendario ICS locales, un alternar del servidor API local, vistas previas de descripciones de tareas y selectores de tareas de teclado más fuertes.
- Los flujos de captura y revisión móvil son más flexibles con captura de bandeja de entrada de Siri, una pestaña de acceso rápido configurable, agrupación de Enfoque opcional, conversión de proyecto a bandeja de entrada y una opción de limpieza de Recordatorios de Apple.
- Los proyectos y las listas de tareas manejan seguimiento de acciones siguientes, reordenamiento de secciones, listas de verificación de referencia, tareas secuenciales programadas y filtrado de proyecto diferido de manera más consistente.
- La sincronización, locale y confiabilidad de configuración se mejoraron con exposición de locale de Holandés, pisos de cobertura de locale, exclusión de latido, reintentos de eliminación de adjunto más seguros, silenciamiento de sincronización sin cambios y manejo de contención de bloqueo de archivo.
- El pulido de escritorio incluye valor predeterminado de tema del sistema, zoom de WebView persistente, revelación de selección de búsqueda global más clara, diseño de división de proyecto mejorado, ajuste de metadatos y acceso API local protegido.

## Escritorio Experience

- Se agregó drag-to-calendar planning for desktop tasks.
- Se agregó local ICS calendar file soporte and allowed Flatpak document-portal vaults.
- Se agregó a local API server setting and protected local API calls with bearer tokens.
- Se agregó desktop task description previews and a Done action in the task editor.
- Mejorado keyboard task selectors, task row actions, task metadata wrapping, hover hints, and narrow task-title layouts.
- Revealed global search selections across areas and refreshed Focus after task edits.
- Se preservó WebView zoom, followed the system theme by default, and clarified the Flatpak global quick-add fallback.
- Mejorado project split sizing, detail-title visibility, inbox project dropdown clipping, and Focus saved-filter empty rows.

## Móvil

- Se agregó Siri inbox capture.
- Se agregó a configurable quick access tab.
- Se agregó optional Focus grouping and clearer mobile task sort controls.
- Se agregó inbox project conversion and an Apple Reminders cleanup option after import.
- Reopened task reminders from notifications more reliably.
- Se mantuvo task editor inputs above the keyboard and stopped controlling Android text selection.
- Se mantuvo the Android widget capture button visible and themed the Android navigation bar.
...
- Se agregó project section reordering and preserved reference checklists.
- Se priorizó scheduled sequential tasks in Focus.
- Hid deferred project tasks from active workflows.
- Filtraed inbox projects by the selected area.
...
- Abreed day view from calendar overflow controls and se mejoró calendar drag accessibility.

## Sync, Settings & Data Safety

- Se agregó an analytics heartbeat opt-out setting.
- Se expuso Dutch in Settings and restored locale coverage floors.
- Se preservó appearance options across settings updates.
- Quieted unchanged mobile sync checks.
- Se honró file sync lock contention and cleaned up desktop sync warnings and paths.
- Se retuvo mobile attachment delete retries more reliably.
- Se compartió fast-sync side-effect checks and typed follow-up helper access.
- Parsed emoji quick-add tags and translated the Inbox Later action.
- Se alineó current característica references in docs and expanded automation helper packaging notes.

## Historial completo de cambios (since `v0.9.3`)
...
- feat(desktop): soporte local ICS calendar files
...
- docs: align current característica references
- refactorización(sync): share fast-sync side effect check
- refactorización(desktop): trim view memo complexity
