...
## Destacados
- Agregado soporte for **local OpenAI-compatible AI endpoints** (for example Ollama/LM Studio/LocalAI/vLLM) via custom base URL.
...
- Móvil UX improvements across iOS and Android (iOS back navigation stabilization, Android checklist keyboard visibility, status bar/theme polish, and inbox/review flow improvements).
...
- Lanzamiento/distribution improvements (official Homebrew cask automation, App Store/TestFlight workflow fixes, and Microsoft Store pipeline updates).

## Core / Sync / Cloud
- Agregado conservative manejo for invalid `deletedAt` timestamps and se corrigió attachment winner URI/metadata merge behavior.
- Hardened cloud write lock queue continuation and se mejoró write serialization safety.
- Agregado SQLite data-integrity validation at DB layer (task/project status validation + JSON field validation) and new sync-oriented composite indexes.
- Shared sync client helpers across desktop/mobile to reduce duplication and keep behavior aligned.
- Mejorado mobile sync config cache TTL and several merge/runtime correctness paths.

## Escritorio / Móvil
- Stabilized iOS drawer/settings back navigation and gesture behavior.
- Corregido Android checklist title input visibility above keyboard and se mejoró Android status bar theming.
- Mejorado mobile inbox processing flow and copilot context suggestions.
- Mejorado weekly review/calendar behavior for scheduled and due tasks.
- Agregado consistent async loading/error feedback in desktop list/projects workflows.
- Mejorado desktop accessibility/i18n coverage and grouped selector subscriptions for better maintainability.

## AI / MCP / Search
- Agregado local OpenAI-compatible endpoint soporte through custom base URL configuration.
...
## Packaging / Docs / CI
- Switched macOS Homebrew install docs to official `homebrew/cask` and se agregó automatic cask bump workflow.
- Eliminado outdated macOS quarantine workaround note after signing pipeline changes.
- Expanded sync/testing wiki guidance and clarified Fluid Recurrence docs.
- Mejorado CI/workflow reliability across iOS/macOS/Windows publishing and metadata pipelines.

## Historial completo de cambios (since 0.6.14)
