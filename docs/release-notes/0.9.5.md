...
- Agregado task location editing and filters across desktop and mobile.
- Indexed task locations in search and se agregó `location:` / `where:` search operators.
- Agregado a task ID search operator for precise lookup.
...
- Preserved task locations in mobile calendar push sync and se eliminó old reference tasks from pushed calendars.
...
- Agregado supported mobile code-block copy behavior.
- Made desktop date pickers close more reliably, honor the configured date format, and align quick-action behavior.
- Restored wheel zoom manejo, recentred the desktop quick-add popup, and se agregó a small text size preset.
...
- Agregado section-scoped sequential project flow.
