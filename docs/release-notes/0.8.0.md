...
- Escritorio project planning is smoother with single-click sidebar selection, a separate archived projects section, better calendar and review contrast, and configurable inline task editor sections.
- Móvil sync is steadier with persistent iOS file-sync bookmarks, clearer cloud-provider grouping, and widgets that size task lists from real widget dimensions.
...
## Escritorio & Planning
- Agregado optional project due dates across the shared data model, JavaScript SQLite adapter, and desktop storage schema, including migration from existing desktop data.
- Agregado a Windows portable distribution mode with local profile and config storage beside the executable.
- Mejorado desktop project navigation with single-click sidebar selection, a separate archived projects section, better calendar and review contrast, and localized focus controls in project and task rows.
- Agregado inline task editor section defaults, kept the Obsidian integration collapsed by default, and refreshed desktop/wiki docs for portable installs and new project fields.

## Móvil, iOS & Android
- Móvil widgets now use actual widget height to decide task limits, and iOS widget payloads are adaptive with a lighter legacy payload for older widget readers.
...
## Data, Sync & Storage
- Agregado project due date indexing and migration for both the Rust and JavaScript SQLite adapters.
- Mejorado merge diagnostics by surfacing large clock skew, tracking invalid deleted timestamps, and making pending remote-write recovery include edits made during backoff.
...
- Mejorado desktop storage safety on Windows with backup-based atomic writes, and emit a warning event when secrets fall back to plaintext because the keyring is unavailable.
- Search now handles archived tasks and projects consistently.

## Docs, CI & Internal Architecture
- Actualizado the wiki MCP page for the new dueDate and reviewAt fields and refreshed desktop Bun test setup to expose the DOM globals Bun misses.
- Corregido release-window TypeScript regressions in shared sync typings and mobile fixture data.
