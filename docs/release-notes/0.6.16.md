...
## Destacados
- Seguridad hardening for cloud sync attachments and auth manejo (safer path decoding, symlink-safe writes, stricter bearer token validation, and se mejoró route-level rate limit scoping).
...
- Móvil UX/performance upgrades (bulk action progress + restore flow, task list `getItemLayout`, task editor memoization, and stricter link attachment validation).
...
- Agregado bearer token format validation before lookup and tightened `/v1/data` rate-limit scoping.
- Corregido desktop and mobile sync orchestration races and cleanup ordering for stability under rapid update events.
- Mejorado core merge behavior for `updatedAt` edge cases and ignored `purgedAt` in content conflict diffing.

## Móvil / Escritorio
- Agregado mobile bulk delete progress UI and restore action.
- Mejorado mobile list performance with measured `getItemLayout` soporte and reduced large modal re-renders.
- Mejorado keyboard/safe-area/task-edit stability across multiple mobile screens.
- Agregado desktop quick-add error toast visibility and prompt confirmation validation for empty input.
- Agregado missing accessibility labels for project/section actions.

## Data Model / Refactorización / Docs
- Centralized default project/area color constants and se eliminó scattered hardcoded defaults.
...
## Historial completo de cambios (since 0.6.15)
